...
module.exports.updateContest = async (req, res, next) => {
  ...
  try {
    const updatedContest = await contestServices.updateContest(req.body, {
      id: contestId,
      userId: req.tokenData.userId,
    });
...
module.exports.setNewOffer = async (req, res, next) => {
  ...
  try {
    const result = await contestServices.createOffer(obj);
...
const rejectOffer = async (offerId, creatorId, contestId) => {
  const rejectedOffer = await contestServices.updateOffer(
...
const resolveOffer = async (
  ...
  const finishedContest = await contestServices.updateContestStatus({
  ...
  const updatedOffers = await contestServices.updateOfferStatus({
    status: db.sequelize.literal(` CASE
...
const approveOffer = async (offerId, email) => {
  const approvedOffer = await contestServices.updateOffer(
...
const disapproveOffer = async (offerId, email) => {
  const disapprovedOffer = await contestServices.updateOffer(
